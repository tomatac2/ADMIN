        ...
        <div class="container mt-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">{{ __("static.state_details") }} : {{ $state->name }}</h4>
                    
                    <a href="{{ route('admin.states.edit', $state->id) }}" class="btn btn-warning text-white">
                        <i class="ri-edit-box-line"></i> {{ __("static.edit") }}
                    </a>
                </div>
<div class="card-body">
                    ...
                    <table class="table table-bordered table-striped">
                        <tbody>
                            ...
                            <tr>
                                <th>{{ __('static.zone_name') }}</th>
                                <td>{{ $state->name }}</td>
                            </tr>
                           
                            
                            <tr>
                                <th> {{ __("static.zone_created") }}</th>
                                <td>{{ $state->created_at }}</td>
                            </tr>
                            <tr>
                                <th>{{ __("static.zone_updated") }}</th>
                    ...
                    @if ($state->cities->count() > 0)
                        <h5 class="mt-4"> {{ __("static.state_cities") }} ({{ $state->cities->count() }})</h5>
                        <ul class="list-group">
                    ...
                    @else
                        <div class="alert alert-info mt-4">{{ __("static.state_no_city") }}</div>
                    @endif
                ...
                <div class="card-footer text-end">
                    <a href="{{ url()->previous() }}" class="btn btn-secondary">
                        <i class="ri-arrow-left-line"></i>   {{ __("static.zone_back") }}
                    </a>
                </div>
            </div>
        </div>
