            ...
            <div class="contentbox">
                <div class="inside">
                    ...
                    <div class="form-group row">
                        <label class="col-md-2" for="faq_category_id">{{ __('static.faqs.category') }} <span>*</span></label>
                        <div class="col-md-10">
                            <select class="form-control" id="faq_category_id" name="faq_category_id" required>
                                <option value="">{{ __('static.select') }}</option>
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}"
                                            {{ (string)old('faq_category_id', $faq->faq_category_id ?? '') === (string)$cat->id ? 'selected' : '' }}>
                                        {{ method_exists($cat,'getTranslation')
                                            ? $cat->getTranslation('name', request('locale', app()->getLocale()))
                                            : $cat->name }}
                                    </option>
                                @endforeach
                            </select>
