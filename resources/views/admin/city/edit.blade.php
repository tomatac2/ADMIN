    ...
    <div class="row dashboard-default">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0">{{ __("static.edit_city") }}</h4>
                <a href="{{ url()->previous() }}" class="btn btn-primary">
                    <i class="ri-arrow-go-back-line"></i> {{ __("static.city_management") }}
                </a>
            </div>
                ...
                <form action="{{ route('admin.cities.update', $city->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="name" class="form-label"> {{ __("static.city_name") }} <span class="text-danger">*</span></label>
                            <input type="text" 
                                class="form-control
                                @error("name")
                                     is-invalid
                            ...
                            <div class="col-md-6 mb-3 ">
                                <label for="countryID" class="form-label">State Name <span class="text-danger">*</span></label>
                                <select name="state_id" id="countryID" class="form-control @error("state_id")
                                     is-invalid
                                @enderror">
                                    <option value="" selected  disabled>Select City</option>
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}" @selected($state->id == $city->state_id)> {{ $state->name }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" class="btn btn-primary me-2">
                            <i class="ri-save-line"></i> {{ __("static.city_save") }}
                        </button>
