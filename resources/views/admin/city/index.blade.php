...
@section('title', __('static.city_management'))
@section('content')
    <div class="row dashboard-default">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0">{{ __("static.city_management") }}</h4>
                <a href="{{ route("admin.cities.create") }}" class="btn btn-primary">
                    <i class="ri-add-line"></i> {{ __("static.add_new_city") }}
                </a>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover table-bordered align-middle">
                        <thead>
                            <tr>
                                <th scope="col">#</th>
                                <th scope="col">{{ __("static.city_name") }}</th>
                                <th scope="col">{{ __("static.state") }}</th>
                                <th scope="col"> {{ __("static.city_created") }}</th>
                                <th scope="col" class="text-center">{{ __("static.city_action") }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @foreach ($cities as $city)
                                ...
                                <td class="d-flex ">
                                     <a href="{{ route("admin.cities.show", $city->id) }}" class="btn btn-sm btn-info text-white me-2" title="{{ __("static.show") }}">
                                       <i class="ri-eye-line"></i>
                                    </a>
                                    <a href="{{ route("admin.cities.edit", $city->id) }}" class="btn btn-sm btn-warning text-white me-2" title="{{ __("static.edit") }}">
                                        <i class="ri-edit-box-line"></i>
                                    </a>
                                   
                                    
                                    <!-- Button trigger modal -->
                                    <button type="button" class="btn btn-sm btn-danger" title="{{ __("static.delete") }}"    data-bs-toggle="modal" data-bs-target="#exampleModal{{ $city->id }}">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="exampleModalLabel">{{ __("static.delete_city") }}</h1>
                                            ...
                                            <form action="{{ route('admin.cities.destroy',$city->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    {{ __( "static.city_confirm_delete") }} <b>{{ $city->name }}</b>
                                                </div>
                                                <div class="modal-footer p-3">
                                                    <button type="button" class="btn btn-sm btn-info" data-bs-dismiss="modal">{{ __('static.close') }}</button>
                                                    <button type="submit" class="btn btn-sm btn-danger">{{ __("static.delete_city") }}</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
