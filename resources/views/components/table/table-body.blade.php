                ...
                @if ($roleName == RoleEnum::ADMIN || $row['created_by_id'] == getCurrentUserId())
                    <label class="switch switch-sm">
                        <input id="status-{{ $row['id'] }}"
                            ...
                            @if ($row[$column['field']]) checked @endif @if (request('filter') === 'trash') disabled @endif>
                        ...
                        @if ($index < $maxVisible)
                            <div class="avatar">
                                @if ($user->profile_image_id)
                                    <img src="{{ $user->profile_image?->original_url }}" alt="image" class="table-image">
                                @else
                                    ...
                                    <li class="{{ $action['class'] }}">
                                        @can($action['permission'])
                                            @if (isset($action['route']))
                                                ...
                                                @else
                                                    <a href="{{ route($action['route'], $row['id']) }}"><span>{{ $action['title'] }}</span></a>
                                                @endif
...
@push('scripts')
    <script>
        function copyToClipboard(event, text) {
            ...
            toastr.success("{{ __('static.toastr.url_copied') }}");
        }
        var email =
